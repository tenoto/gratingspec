...
def grating_spectrum_gauss(df_lines,
...
def grating_spectrum_Fe(fe_Hlike_fits,
	fname_pdf='spectrum.pdf',fname_jpg='spectrum.jpg',
	linewidthfactor=8.0):
	...
	hdu = pyfits.open(fe_Hlike_fits)
	for i in range(len(hdu["PEAKEMIS"].data)):
		wavelength_A = hdu["PEAKEMIS"].data[i]["Lambda"]
		line_keV = 12.398 / wavelength_A # keV
		PeakEmissivity = hdu["PEAKEMIS"].data[i]["PeakEmissivity"]
		print(i,wavelength_A,line_keV,PeakEmissivity)
		ax.plot([np.log10(line_keV*1000.0), np.log10(line_keV*1000.0)], 
			[0, 1], color="white", linewidth=0.8*np.log10(PeakEmissivity/1e-20))
	...
	for index, row in df_lines.iterrows():
		...
		color = wavelength_to_rgb(wavelength_nm)
		#ax.plot([wavelength, wavelength], [0, 1], color=color, linewidth=1.0*intensity)
		ax.plot([np.log10(energy_eV), np.log10(energy_eV)], 
		#ax.plot([energy_eV, energy_eV], 
			[0, 1], color=color, linewidth=1.0*intensity)
    
	#ax.secondary_xaxis('top', functions=(eV_to_wavelength_nm, wavelength_nm_to_eV))
	"""
	plt.savefig(fname_pdf, format='pdf', facecolor='black')
	plt.savefig(fname_jpg, format='jpg', facecolor='black')	
...
fname_pdf = "Fe_Hlike.pdf"
fname_jpg = "Fe_Hlike.jpg"
fe_Hlike_fits = "data/apec_v3.0.9_linelist_fe_Hlike.fits"
grating_spectrum_Fe(fe_Hlike_fits,fname_pdf=fname_pdf,fname_jpg=fname_jpg)
