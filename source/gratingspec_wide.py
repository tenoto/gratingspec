...
fname_pdf = os.path.splitext(os.path.basename(fname_csv))[0] + '_specwide.pdf'
fname_jpg = os.path.splitext(os.path.basename(fname_csv))[0] + '_specwide.jpg'
df_lines = read_csv_to_dflines(fname_csv) 
grating_spectrum(df_lines,fname_pdf=fname_pdf,fname_jpg=fname_jpg)

"""
wavelength = np.linspace(300,800,num=5000)
intens_relative = np.full(len(wavelength),1.0)
df_lines = pd.DataFrame(
	data={'wavelength':wavelength,
	'intens_relative':intens_relative})
print(df_lines)
grating_spectrum(df_lines,outpdf='full_spectrum.pdf',outjpg='full_spectrum.jpg')
"""